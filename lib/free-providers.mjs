...
const OPENAI_COMPAT = [
  ...
  },
  {
    ...
    extraHeaders: { 'HTTP-Referer': 'https://ai-minaret.vercel.app', 'X-Title': 'AI-Minaret' },
  },
  {
    id: 'nvidia',
...
// Two rotation orders, not one — so "primary" and "secondary" roles land on
// different providers/models by default (mirrors the local setup, where
// primary = your local Gemma and secondary = a different model entirely, for
// genuinely independent reasoning across layers). Each still falls through
// on missing key or error; PRIMARY favors the fastest free tiers first.
 ...
 * @param {string} role - 'primary' or 'secondary'; each uses a different
 *   rotation order so the two roles land on different models when possible.
...
export async function rotateFreeProviders(messages, maxTokens = 1200, role = 'primary') {
  const attempts = [];
  const order = role === 'secondary' ? ORDER_SECONDARY : ORDER_PRIMARY;
